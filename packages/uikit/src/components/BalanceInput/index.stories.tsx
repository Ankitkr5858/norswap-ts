...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"NSWAP" | "USD">("NSWAP");
  const conversionUnit = editingUnit === "NSWAP" ? "USD" : "NSWAP";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "NSWAP" ? "USD" : "NSWAP";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "NSWAP") {
      ...
      setValues({
        NSWAP: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
