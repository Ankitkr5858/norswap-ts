...
import { Menu as UikitMenu } from '@nswap/uikit'
...
const Menu = (props) => {
  ...
  const lpToken = farmsConfig.find(item => item.token.symbol === serializedTokens.nswap.symbol && item.quoteToken.symbol === serializedTokens.wnrk.symbol);
  
  // useFarmFromLpSymbol(lpToken.lpSymbol);
  const lpTokenPriceInUsd = useLpTokenPrice(lpToken.lpSymbol)
  const bnbToken = tokens.wnrk;
  ...
  return (
    <UikitMenu
      ...
      buyCakeLabel={t('Buy NSWAP')}
      buyBNBComponent={<BuyBNB currency={bnbToken} lpTokenPriceInUsd={lpTokenPriceInUsd} buyBNBLabel={t('Buy NSWAP')} />}
      {...props}
    />
  )
}
