...
import { JSBI, Pair, Percent } from '@danielvindax/norswap-sdk'
import {
  Button,
  Text,
  ChevronUpIcon,
  ...
  return (
    <>
      {userPoolBalance && JSBI.greaterThan(userPoolBalance.raw, BIG_INT_ZERO) ? (
            ...
            <AutoColumn gap="16px">
              ...
              <FixedHeightRow>
                <RowFixed>
                  ...
                  <Text small color="#000000">
                    {currency0.symbol}-{currency1.symbol} LP
                  </Text>
                </RowFixed>
                <RowFixed>
                  <Flex flexDirection="column" alignItems="flex-end">
                    <Text color="#000000">{userPoolBalance ? userPoolBalance.toSignificant(4) : '-'}</Text>
                    {Number.isFinite(totalUSDValue) && (
                      <Text small color="#000000">{`(~${totalUSDValue.toLocaleString(undefined, {
              ...
              <AutoColumn gap="4px">
                {poolData && (
                  <FixedHeightRow>
                    <TooltipText ref={targetRef} color="#000000" small>
                      {t('LP reward APR')}:
                    </TooltipText>
                    {tooltipVisible && tooltip}
                    <Text color="#000000">{formatAmount(poolData.lpApr7d)}%</Text>
                  </FixedHeightRow>
                )}
                <FixedHeightRow>
                  <Text color="#000000" small>
                    {t('Share of Pool')}:
                  </Text>
                  <Text color="#000000">{poolTokenPercentage ? `${poolTokenPercentage.toFixed(6)}%` : '-'}</Text>
                </FixedHeightRow>
                <FixedHeightRow>
                  <Text color="#000000" small>
                  ...
                  {token0Deposited ? (
                    <RowFixed>
                      <Text ml="6px" color="#000000">{token0Deposited?.toSignificant(6)}</Text>
                    </RowFixed>
                  ) : (
                    '-'
                  )}
                ...
                <FixedHeightRow>
                  ...
                  {token1Deposited ? (
                    <RowFixed>
                      <Text ml="6px" color="#000000">{token1Deposited?.toSignificant(6)}</Text>
                    </RowFixed>
                  ) : (
                    '-'
                  )}
        ...
        <LightCard>
          <Text color="#000000" fontSize="14px" style={{ textAlign: 'center' }}>
            {' '}
            {t(
