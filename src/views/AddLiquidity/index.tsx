...
import { Currency, currencyEquals, ETHER, TokenAmount, WETH } from '@danielvindax/norswap-sdk'
import { Button, Text, AddIcon, CardBody, Message, useModal, TooltipText, useTooltip } from '@nswap/uikit'
  ...
  const handleCurrencyBSelect = useCallback(
    (currencyB_: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        router.replace(`/add/${currencyIdA || 'NRK'}/${newCurrencyIdB}`, undefined, { shallow: true })
      }
    },
      ...
      <AppBody>
        ...
        <CardBody>
          <AutoColumn gap="20px">
            {noLiquidity && (
              <ColumnCenter>
                <Message variant="warning">
                  <div>
                    <Text bold mb="8px" color="#000000">
                      {t('You are the first liquidity provider.')}
                    </Text>
                    <Text color="#000000" mb="8px">{t('The ratio of tokens you add will set the price of this pool.')}</Text>
                    <Text color="#000000">{t('Once you are happy with the rate click supply to review.')}</Text>
                  </div>
                </Message>
              </ColumnCenter>
            )}
            ...
            {pair && poolData && (
              <RowBetween>
                <TooltipText ref={targetRef} bold fontSize="12px" color="#F4E17A">
                  {t('LP reward APR')}
                </TooltipText>
                {tooltipVisible && tooltip}
                <Text bold color="#F4E17A">
