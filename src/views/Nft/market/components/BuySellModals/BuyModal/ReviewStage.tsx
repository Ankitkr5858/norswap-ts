...
import { Flex, Text, Button, ButtonMenu, ButtonMenuItem, Message, Link } from '@nswap/uikit'
  ...
  return (
    <>
      <Flex px="24px" pt="24px" flexDirection="column">
        ...
        <BorderedBox>
          ...
          <ButtonMenu
            ...
            variant="subtle"
          >
            <ButtonMenuItem>NRK</ButtonMenuItem>
            <ButtonMenuItem>WNRK</ButtonMenuItem>
          </ButtonMenu>
          <Text small color="textSubtle">
            {t('Total payment')}
          </Text>
          ...
          <Text small color="textSubtle">
            {t('%symbol% in wallet', { symbol: paymentCurrency === PaymentCurrency.NRK ? 'NRK' : 'WNRK' })}
          </Text>
          {!account ? (
          ...
          <Message p="8px" variant="danger">
            <Text>
              {t('Not enough %symbol% to purchase this NFT', {
                symbol: paymentCurrency === PaymentCurrency.NRK ? 'NRK' : 'WNRK',
              })}
            </Text>
          </Message>
        )}
        <Flex alignItems="center">
          <Text my="16px" mr="4px">
            {t('Convert between NRK and WNRK for free')}:
          </Text>
          <Button
            as={Link}
            p="0px"
            ...
            external
            variant="text"
            href="/swap?inputCurrency=NRK&outputCurrency=0x9c294d255fBBf8E7bBb510D7E29e99c22D5962a8"
          >
            {t('Convert')}
          </Button>
        </Flex>
      ...
      <Flex px="24px" pb="24px" flexDirection="column">
        ...
        <Button as={Link} external style={{ width: '100%' }} href="/swap?outputCurrency=NRK" variant="secondary">
          {t('Get %symbol1% or %symbol2%', { symbol1: 'NRK', symbol2: 'WNRK' })}
        </Button>
      </Flex>
    </>
  )
