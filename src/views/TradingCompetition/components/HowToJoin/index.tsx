...
import { Flex, Text, Heading } from '@nswap/uikit'
    ...
    <Flex flexDirection="column" maxWidth="736px">
      ...
      <HowToCard number={1} title={t('Get Ready')}>
        <Text fontSize="14px" color="textSubtle">
          {t('Set up your')}{' '}
          {account ? (
            <StyledLink href={`${nftsBaseUrl}/profile/${account.toLowerCase()}`}>{t('NSWAP Profile')}</StyledLink>
          ) : (
            t('NSWAP Profile')
          )}
          {', '}
      ...
      <HowToCard number={2} title={t('Battle Time')}>
        <Text fontSize="14px" color="textSubtle">
          {t(
            'Trade DAR/NRK, NSWAP/NRK, and NSWAP/NUSD during the battle period to raise both your and your team’s score. See the Rules section below for more.',
          )}
        </Text>
      </HowToCard>
