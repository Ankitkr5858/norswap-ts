...
import { Currency, currencyEquals, ETHER, Percent, WETH } from '@danielvindax/norswap-sdk'
import {
  Button,
  Text,
  AddIcon,
                  ...
                  {chainId && (oneCurrencyIsWBNB || oneCurrencyIsBNB) ? (
                    <RowBetween style={{ justifyContent: 'flex-end', fontSize: '14px' }}>
                      {oneCurrencyIsBNB ? (
                        ...
                        <StyledInternalLink
                          href={`/remove/${
                            currencyA && currencyEquals(currencyA, WETH[chainId]) ? 'NRK' : currencyIdA
                          }/${currencyB && currencyEquals(currencyB, WETH[chainId]) ? 'NRK' : currencyIdB}`}
                        >
                          {t('Receive NRK')}
                        </StyledInternalLink>
                      ) : null}
