...
import { Modal, Text, Flex, BalanceInput, Box, Button, LogoRoundIcon } from '@nswap/uikit'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.nswap.address)
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient NSWAP balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% NSWAP or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      <ExistingInfo>
        <Flex justifyContent="space-between">
          ...
          <Text>{t('%num% NSWAP', { num: getBalanceNumber(amount).toLocaleString() })}</Text>
        </Flex>
        <Flex justifyContent="space-between">
      ...
      <InnerContent>
        <Flex justifyContent="space-between" alignItems="center" pb="8px">
          ...
          <Flex>
            ...
            <Text bold>NSWAP</Text>
          </Flex>
        </Flex>
        {isFirstBid && (
          <Text pb="8px" small>
            {t('First bid must be %initialBidAmount% NSWAP or more.', { initialBidAmount })}
          </Text>
        )}
        <BalanceInput
        ...
        <Text color="textSubtle" small mt="24px">
          {t('If your bid is unsuccessful, you’ll be able to reclaim your NSWAP after the auction.')}
        </Text>
      </InnerContent>
    </StyledModal>
  )
