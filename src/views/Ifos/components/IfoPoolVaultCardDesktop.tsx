        ...
        <StyledTokenContent justifyContent="space-between" alignItems="center">
          <Box ml="8px">
            ...
            <Text color="textSubtle" fontSize="14px">
              {t('Stake NSWAP to participate in IFO')}
            </Text>
          </Box>
          <TokenPairImage width={64} height={64} primaryToken={tokens.nswap} secondaryToken={tokens.nswap} />
        </StyledTokenContent>
      </CardHeader>
      <StyledCardBody>
        <Message variant="warning" mb="37px">
          <Flex flexDirection="column">
            <Text mb="16px">
              {t(
                'This is the old IFO NSWAP pool. Check out the brand new NSWAP pool to learn how to earn NSWAP rewards with higher APY while enjoying other benefits.',
              )}
            </Text>
            <Flex ml="-34px">
              <Button onClick={handleOnClick} width="100%">
                {isShowMigrationButton ? t('Migrate') : t('Go to new NSWAP pool')}
              </Button>
            </Flex>
          </Flex>
        </Message>
        ...
        <Box mt="24px">
          <Flex alignItems="center" justifyContent="space-between">
            <Text fontSize="14px">{`${t('Recent NSWAP profit')}:`}</Text>
            <Balance
              bold
              fontSize="14px"
