...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === tokens.nswap
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of NSWAP up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max NSWAP entry is capped by minimum between your average NSWAP balance in the IFO NSWAP pool, or the pool’s hard cap. To increase the max entry, Stake more NSWAP into the IFO NSWAP pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max NSWAP entry is capped by your average NSWAP balance in the IFO NSWAP pool. To increase the max entry, Stake more NSWAP into the IFO NSWAP pool',
        )
      : t(
          'For the unlimited sale, Max NSWAP entry is capped by your average NSWAP balance in the IFO NSWAP pool. To increase the max entry, Stake more NSWAP into the IFO NSWAP pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. NSWAP entry') : t('Max. token entry')
...
const IfoCardDetails: React.FC<IfoCardDetailsProps> = ({ isEligible, poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  // unlimited pool just show the nrk left
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('NSWAP to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
