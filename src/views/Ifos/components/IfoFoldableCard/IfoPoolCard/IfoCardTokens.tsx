...
/* const CommitTokenSection: React.FC<TokenSectionProps & { commitToken: Token }> = ({ commitToken, ...props }) => {
  if (commitToken.equals(cakeBnbLpToken)) {
    return <TokenSection primaryToken={tokens.nswap} secondaryToken={tokens.wnrk} {...props} />
  }
...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough NSWAP to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your NSWAP.',
    ),
    { placement: 'bottom' },
  )
  ...
  const renderTokenSection = () => {
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate NSWAP Profile to take part in next IFO‘s!')
                : t('You need an active NSWAP Profile to take part in an IFO!')}
            </MessageText>{' '}
    ...
    // (ifo.version === 3 || (ifo.version === 3.1 && poolId === PoolIds.poolUnlimited)) && getBalanceNumber(nrk) === 0)
    if (ifo.version === 3 || (ifo.version === 3.1 && poolId === PoolIds.poolUnlimited)) {
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">
              {t('You don’t have any average NSWAP balance available to commit in the IFO NSWAP pool.')}
            </MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, stake some NSWAP in the IFO NSWAP pool!')}
              </Text>
