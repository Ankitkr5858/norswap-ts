...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@nswap/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%NSWAP Squad NFT%'

  const rawText = t(`Set %NSWAP Squad NFT% as NSWAP Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`http://norswap.io/nfts/collections/${admissionProfile}`}
          >
            {t('NSWAP Squad NFT')}
          </LinkExternal>
          <br />
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%NSWAP Profile%'

  const rawText = t(`Reach %point% or more %NSWAP Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`http://norswap.io/nfts/profile/${account}/achievements/`}
          >
            {t('NSWAP Profile')}
          </LinkExternal>
        </>
