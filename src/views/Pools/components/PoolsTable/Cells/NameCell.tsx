      ...
      <CellContent>
        {showStakedTag &&
          (vaultKey ? (
            ...
            <Text fontSize="12px" bold color={isFinished ? 'failure' : '#000000'} textTransform="uppercase">
              {t('Staked')}
            </Text>
          ))}
        <Text color="#ffffff" bold={!isMobile} small={isMobile}>
          {title}
        </Text>
        {showSubtitle && (
...
const stakedStatus = {
  [VaultPosition.None]: { text: '', color: '#000000' },
  [VaultPosition.Locked]: { text: 'Locked', color: '#000000' },
  [VaultPosition.LockedEnd]: { text: 'Locked End', color: '#000000' },
