...
import { Flex, Text, Box, TooltipText, useTooltip } from '@nswap/uikit'
    ...
    <LightGreyCard>
      <Flex justifyContent="space-between" mb="16px">
        <Box>
          ...
          <BalanceWithLoading  bold fontSize="16px" value={currentLockedAmount} decimals={5} />
          <BalanceWithLoading
        ...
        <Box>
          ...
          <Text color={position >= VaultPosition.LockedEnd ? '#D67E0A' : ''} bold fontSize="16px">
            {position >= VaultPosition.LockedEnd ? t('Unlocked') : remainingTime}
          </Text>
          <Text color={position >= VaultPosition.LockedEnd ? '#D67E0A' : ''} fontSize="12px">
      ...
      {![VaultPosition.LockedEnd, VaultPosition.AfterBurning].includes(position) && (
        <Flex alignItems="center" justifyContent="space-between">
          ...
          <BalanceWithLoading  bold fontSize="16px" value={parseFloat(lockedApy)} decimals={2} unit="%" />
        </Flex>
      )}
      ...
      <Flex alignItems="center" justifyContent="space-between">
        <Text  textTransform="uppercase" bold fontSize="12px">
          {t('Recent NSWAP profit')}
        </Text>
        <BalanceWithLoading  bold fontSize="16px" value={earningTokenBalance} decimals={5} />
      </Flex>
