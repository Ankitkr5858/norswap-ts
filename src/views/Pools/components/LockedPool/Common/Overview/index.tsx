  ...
  return (
    <>
      <Box>
        ...
        <LightGreyCard>
          <BalanceRow title={t('NSWAP to be locked')} value={lockedAmount} newValue={newLockedAmount} decimals={2} />
          <BalanceRow
            title="apy"
            unit="%"
          ...
          />
          <DateRow
            color={_toNumber(newDuration) ? 'failure' : 'textSubtle'}
            title={t('Unlock on')}
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t('You will be able to withdraw the staked NSWAP and profit only when the staking position is unlocked')}
            </MessageText>
          </Message>
        </Box>
      )}
