...
import { Pair, TokenAmount, CurrencyAmount, Trade } from '@danielvindax/norswap-sdk'
...
describe('Trade', () => {
  describe('#useAllCommonPairs', () => {
    ...
    it('should filter only exist Pair', () => {
      mockUsePairs.mockReturnValue([
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.wnrk, '1'), new TokenAmount(mainnetTokens.nswap, '1')),
        ],
        [
          PairState.INVALID,
          new Pair(new TokenAmount(mainnetTokens.nusd, '1'), new TokenAmount(mainnetTokens.nswap, '1')),
        ],
        [
          PairState.LOADING,
          new Pair(new TokenAmount(mainnetTokens.nusd, '1'), new TokenAmount(mainnetTokens.wnrk, '1')),
        ],
        [PairState.EXISTS, null],
      ])

      const { result } = renderHook(() => {
        const pairs = Trades.useAllCommonPairs(mainnetTokens.wnrk, mainnetTokens.nswap)
        return {
          pairs,
        }
      })
    ...
    it('should filter out duplicated Pair', () => {
      mockUsePairs.mockReturnValue([
        ...
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.nswap, '1'), new TokenAmount(mainnetTokens.wnrk, '1')),
        ],
        [PairState.EXISTS, null],
      ])
    ...
    it('should get all pair combinations wnrk, nswap', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wnrk, mainnetTokens.nswap)
      })

      expect(mockUsePairs).toMatchSnapshot()
    })

    it('should get all pair combinations, wnrk, wnrk', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wnrk, mainnetTokens.wnrk)
      })
  ...
  describe('#useTradeExactIn/Out', () => {
    ...
    it('should call with maxHops 1 with singleHopOnly', () => {
      const allowPairs = [new Pair(new TokenAmount(mainnetTokens.wnrk, '1'), new TokenAmount(mainnetTokens.nswap, '1'))]
      const argA = CurrencyAmount.ether('1000000')
      const argB = mainnetTokens.nswap
      renderHook(
        () => {
