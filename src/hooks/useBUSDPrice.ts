...
const { wnrk: WBNB, nusd } = tokens

/**
 * Returns the price in NUSD of the input currency
 * @param currency currency to compute the NUSD price of
 */
...
export const useCakeBusdPrice = (): Price | undefined => {
  const cakeBusdPrice = useBUSDPrice(tokens.nswap)
...
export const useBNBBusdPrice = (): Price | undefined => {
  const bnbBusdPrice = useBUSDPrice(tokens.wnrk)
  return bnbBusdPrice
}
