// @ts-nocheck
// @ts-nocheck
import { Currency, currencyEquals, ETHER, WETH } from '@danielvindax/norswap-sdk'
      ...
      return {
        ...
        execute:
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, {
                    summary: `Wrap ${inputAmount.toSignificant(6)} NRK to WNRK`,
                    type: 'wrap',
                  })
                } catch (error) {
            ...
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient NRK balance'),
      }
    }
          ...
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, { summary: `Unwrap ${inputAmount.toSignificant(6)} WNRK to NRK` })
                } catch (error) {
                  console.error('Could not withdraw', error)
                }
              }
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient WNRK balance'),
      }
    }
