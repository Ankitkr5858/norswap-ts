...
describe('Config farms', () => {
  ...
  it.each(farmsToTest)('Farm #%d has an unique address', (pid, farm) => {
    const duplicates = farms.filter((f) => farm.lpAddresses[81041] === f.lpAddresses[81041])
    expect(duplicates).toHaveLength(1)
  })
  ...
  it.each(farmsToTest)('Farm %d has the correct token addresses', async (pid, farm) => {
    ...
    const lpContract = getLpContract(farm.lpAddresses[81041])
  ...
  const START_PID = 2
  const FACTORY_ADDRESS = '0x349EbAFd6E67A0880Fe1680A3511AEc23Ade1684'
