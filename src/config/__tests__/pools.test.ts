...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[81041] === p.contractAddress[81041])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'NRK'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'NRK'))(
