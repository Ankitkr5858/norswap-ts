...
import { Currency, CurrencyAmount, TokenAmount, Trade, Token, Price, ETHER } from '@danielvindax/norswap-sdk'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === ETHER ? 'NRK' : '',
        }),
      )
    },
    [dispatch],
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'NRK') return 'NRK'
    if (valid === false) return 'NRK'
  }
  return ''
}
