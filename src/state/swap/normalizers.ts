    ...
    ?.map((pairPrice) => ({
      ...
      value:
        activeToken === pairPrice?.token0Id
          ? (pairPrice.reserve0 ? pairPrice.reserve1 / pairPrice.reserve0 : 0)
          : (pairPrice.reserve1 ? pairPrice.reserve0 / pairPrice.reserve1 : 0),
    }))
    .reverse()
...
export const normalizeDerivedPairDataByActiveToken = ({
  ...
  pairData?.map((pairPrice) => ({
    ...
    value:
      activeToken === pairPrice?.token0Id
        ? (pairPrice.token1DerivedBNB ? pairPrice.token0DerivedBNB / pairPrice.token1DerivedBNB : 0)
        : (pairPrice.token0DerivedBNB ? pairPrice.token1DerivedBNB / pairPrice.token0DerivedBNB : 0),
  }))
